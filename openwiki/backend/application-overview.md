---
type: backend-overview
title: Backend application overview
description: Composition-root map of the FastAPI backend, including package boundaries, mounted endpoints, service seams, and the major runtime subsystems.
...
These modules are described in more detail in [Backend auth and tenancy](auth-and-tenancy.md).
...
See [Helpdesk workflow](helpdesk-workflow.md).
...
See [Agent definitions](agent-definitions.md).
...
See [Knowledge pipeline](knowledge-pipeline.md).
...
See [Domains and endpoints](domains-and-endpoints.md).
