...
description: Cross-system map of the Foundry Assured repository, including deployment modes, domain model, and the live versus hosted runtime paths.
tags: [architecture, backend, frontend, infrastructure]
---
...
The runtime center of gravity is the backend in [`apps/backend`](../backend/application-overview.md), but the repository is intentionally split into reusable seams so the same logical domains can run either as a live AG-UI workflow or as hosted agents.
...
- **`dedicated`**: single-tenant customer-cloud deployment operated through managed application packaging and Lighthouse delegation. Runtime code still uses `SingleTenantConfigProvider`; the main difference is packaging and operational control. See [Dedicated mode infrastructure](../infrastructure/dedicated-mode.md).
...
See [Hosted agents overview](../hosted-agents/overview.md) and domain-specific hosted pages for the exact tradeoffs.
...
See [Frontend application overview](../frontend/application-overview.md).
...
See [Backend application overview](../backend/application-overview.md).
...
See [Hosted agents overview](../hosted-agents/overview.md).
...
See [Infrastructure deployment](../infrastructure/deployment.md) and [Automation and release](../operations/automation-and-release.md).
...
- For backend package ownership and route composition, read [Backend application overview](../backend/application-overview.md).
- For auth, OBO, and multi-tenant mode, read [Backend auth and tenancy](../backend/auth-and-tenancy.md).
- For AG-UI and frontend runtime behavior, read [Frontend domain console](../frontend/domain-console.md).
- For infrastructure packaging, read [Infrastructure deployment](../infrastructure/deployment.md).
- For assurance and repository automation, read [Evaluation harness](../assurance/evaluation-harness.md) and [Automation and release](../operations/automation-and-release.md).
