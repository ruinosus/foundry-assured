...
title: Wiki automation decision
description: Summary of the ADR that moved wiki freshness from a noisy PR gate to a deterministic trigger, adopted OpenWiki for regeneration, and kept the repository's own bundle and assurance seams.
tags: [architecture, adr, wiki, automation]
openwiki:
  roles: [architecture, workflow]
  change_kinds: [automation, decision-record]
  source_paths: [docs/adr/ADR-016-openwiki-closes-the-freshness-loop.md, docs/adr/ADR-012-reuse-upstream-deep-wiki-tooling.md, .github/workflows/wiki-freshness.yml, .github/workflows/wiki-regen.yml]
  symbols: [ADR-012, ADR-016]
  test_paths: [apps/backend/eval/wiki_freshness_test.py, apps/backend/eval/wiki_fidelity_test.py]
  invariants: [The repository outsources generation mechanics to OpenWiki but retains deterministic drift detection, docbundle adaptation, and fidelity gating as local trust boundaries.]
  validation_commands: [cd /home/runner/work/foundry-assured/foundry-assured && git log --oneline -n 5 -- docs/adr .github/workflows apps/backend/app/knowledge apps/backend/eval]
---
# Wiki automation decision

Consult this page when a proposed change questions the overall shape of the self-wiki maintenance system rather than a single file. The operational details live in the [wiki refresh loop](../automation/wiki-refresh-loop.md); this page explains **why** the loop is structured that way.
...
That relationship is realized by the [wiki refresh loop](../automation/wiki-refresh-loop.md).
...
This page intentionally links back to the [wiki refresh loop](../automation/wiki-refresh-loop.md) because the adapter is not only format translation; it also carries trust decisions about page ordering, front-matter stripping, internal-link flattening, and manifest provenance.
...
- Behavior checks: follow the focused commands listed on the [wiki refresh loop](../automation/wiki-refresh-loop.md).
