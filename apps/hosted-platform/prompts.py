"""Compose the hosted-platform prompt from the declarative DNA scope.
...
Composing the ``platform`` Agent (not an inline paraphrase) means the HITL
never-claim-a-write rule arrives as the wired ``## Guardrail: no-write-claims``
section — fixing the format drift the old inline copy had.
"""
...
# source tree (apps/hosted-platform -> parents[1] == apps, then backend/.dna).
...
PLATFORM_INSTRUCTIONS = load_prompts(_DNA_SCOPE, base_dir=str(_resolve_base_dir()))["platform"]
