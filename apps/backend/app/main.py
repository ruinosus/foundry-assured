...
one loop that dispatches by `kind` (workflow/grounded/tool). O endpoint MCP NÃO mora
mais aqui: ele é `apps/mcp` desde a Fase 0c (ADR-027). Business logic lives in
...
# all. mount_domains() now walks the static topology only — see domains.public.DOMAIN_KINDS.
tenancy.install()
...
# O `/mcp` NÃO É MAIS SERVIDO AQUI (Fase 0c, ADR-027). O servidor MCP é uma unidade de deploy
# própria — `apps/mcp`, sobre FastMCP 4 — e o monolito não tem mais um sub-app montado em
# prefixo, nem as rotas `.well-known` reerguidas na raiz. Duas superfícies servindo a MESMA tool
# era a divergência que este projeto mais teme: uma delas podia passar a decidir diferente sobre
# o que o usuário pode ver, sem erro nenhum. Agora existe uma só.
