...
lazy, cached, fail-loud, returns clean prompts).
...
- The Microsoft **wiki-page-writer** depth rules (formerly read raw by
  ...
  composes it between the persona preamble and the pipeline adaptations.
...
# or agent, so a run that starts is a run with real prompts.
_prompts = load_prompts(_DNA_SCOPE, base_dir=str(_resolve_base_dir()))

WIKI_PLANNER_INSTRUCTIONS = _prompts["wiki-planner"]
...
WIKI_PAGE_WRITER_INSTRUCTIONS = _prompts["wiki-page-writer"]
WIKI_VERIFIER_INSTRUCTIONS = _prompts["wiki-verifier"]
