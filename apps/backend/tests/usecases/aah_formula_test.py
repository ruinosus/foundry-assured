...
from app.modules.usecases.internal.outcomes import DEFAULT_ASSUMPTION, outcomes
...
def _rodar(*, conversas, com_refs, referencias, escalados, hora):
    ...
    return outcomes(
        {"id": "exemplo", "agents": []},
        {**DEFAULT_ASSUMPTION, "hourly_cost": hora, "currency": "USD"},
    )
...
def main() -> int:
    ...
    if DEFAULT_ASSUMPTION["minutes_per_reference"] != 6.0:
        falhas.append("  multiplicador default deixou de ser 6 min (o publicado pela Microsoft)")
    if (DEFAULT_ASSUMPTION["resolved_weight"], DEFAULT_ASSUMPTION["unresolved_weight"]) != (1.0, 0.7):
