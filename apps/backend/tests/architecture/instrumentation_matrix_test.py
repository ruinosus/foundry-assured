...
def _autenticada(deps: list[str]) -> bool:
    ...
    Sem sensibilidade a maiúscula porque nem toda evidência é nome de função: numa rota de
    sub-app ASGI (o MCP) quem exige identidade é uma CLASSE de middleware — `RequireAuthMiddleware`
    — e comparar `"auth" in "RequireAuthMiddleware"` daria falso negativo por causa de uma letra.
    """
    return any(
        "user" in n or "auth" in n or "require" in n for n in (d.lower() for d in deps)
    )
