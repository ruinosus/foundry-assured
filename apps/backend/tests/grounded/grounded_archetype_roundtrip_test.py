"""Task 10 (Part 1) — A-vs-B ACL round-trip THROUGH the unified `/techdocs` HTTP endpoint.
...
`retrieve()` in isolation (that's `eval.retrieval_acl_parity_test`), but the real HTTP path:
    ...
    POST /techdocs  →  auth dependency (bearer → User) → current_user() captured in the endpoint →
    stream_grounded(body, techdocs_domain, user)  →  retrieve() (native searchIndex retrieve +
    per-user x-ms-query-source-authorization OBO token) → synthesize → the AG-UI `sources` CUSTOM event.

For each of two REAL Entra users we POST the same grounded probe to the LIVE unified `/techdocs`
...
mention the topic "telemetria" without CITING the confidential doc; only the source set is the ACL fact.
  - User A (confidential group) MUST cite TECHDOCS_CONFIDENTIAL_SOURCE (`techdocs-mcp-telemetry`).
...
class _Creds(BaseSettings):
    ...
    techdocs_acl_probe: str = "Como funciona a telemetria e a observabilidade do TechDocs?"
...
def _ask(backend: str, token: str, probe: str) -> tuple[int, list[str], str | None]:
    """POST the grounded turn to the LIVE /techdocs; return (answer_chars, cited_sources, run_error).
...
def main() -> int:
    ...
    if not all([tid, api, secret, a, b, pw, conf]):
        print("⏭️  SKIP: unified /techdocs round-trip needs ENTRA_TENANT_ID + ENTRA_API_CLIENT_ID/SECRET "
              "+ TECHDOCS_TEST_USER_A/B + TECHDOCS_TEST_PASSWORD + TECHDOCS_CONFIDENTIAL_SOURCE.")
        return 0

    print(f"unified /techdocs A-vs-B round-trip @ {backend.rstrip('/')}/techdocs, probe='{probe}'")
    ...
    if ea or eb:
        print(f"\n❌ FAIL: RUN_ERROR on the live /techdocs (A={ea!r} B={eb!r}) — the grounded endpoint errored.")
        return 1
    if not (ca and cb):
        print("\n❌ FAIL: an answer came back empty — the synthesis didn't stream through the endpoint.")
        return 1
    if not a_has:
        print("\n❌ FAIL: cleared User A did NOT cite the confidential doc THROUGH /techdocs — "
              "endpoint auth / OBO / retrieve() / sources-event is broken.")
        return 1
    if b_has:
        print("\n❌ FAIL: public-only User B cited the confidential doc THROUGH /techdocs — ACL LEAK.")
        return 1
    print("\n✅ PASS: the unified /techdocs endpoint enforces per-user document ACL end-to-end — "
