...
def main() -> int:
    ...
    print("\n7. o frontmatter do corpus real não vaza para o índice")
    # A CONFORMIDADE §11 do corpus (todo documento com `type`) é checada por
    # `tests.knowledge.okf_conformance_test`, com o verificador da própria spec — ela é regra do
    # FORMATO, e um segundo checador nosso divergiria dela no primeiro caso de borda.
    #
    # O que sobra aqui é o que é NOSSO: o frontmatter que existe para ser conformante NÃO pode
    # aparecer no corpo indexado. Se aparecer, o corpus de retrieval ganha YAML e o modelo passa
    # a citar `type:` como se fosse conteúdo — a mesma falha que o passo 1 guarda para os
    # arquivos sintéticos, aqui sobre os arquivos de verdade.
    ...
    tem_yaml = [n for n, d in blobs_reais if "type: runbook" in d.decode("utf-8")[:200]]
    checar("nem no começo do corpo", not tem_yaml, f"com YAML: {tem_yaml}")
