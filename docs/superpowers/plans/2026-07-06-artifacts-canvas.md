...
- **Live HTML:** the backend state is a **flat string** field `html` (Chunk 1 uses `update_artifact(html: str)` with `state_schema={"html":{"type":"string"}}` + `predict_state_config` → tool arg `html`, so partial deltas stream token-by-token). `agent.subscribe({ onStateSnapshotEvent, onStateDeltaEvent })` → maintain an `html` React state. Snapshot: read `event.snapshot.html`. Delta: the JSON-Patch targets `/html` (apply it, or just take the patched value as the new full string). If the installed version also exposes `agent.state`, reading `agent.state.html` on each `onEvent` is an acceptable simpler equivalent — verify which is populated and use it; keep the subscribe tap as the guaranteed path. Throttle `setHtml` with `requestAnimationFrame`.
