...
- [ ] **Step 3: implement** in `tools.py`:
  ...
  > **Two self-hosted guardrails (the #1 correctness property):** (1) use a **literal `else:`** for the non-shared branch — NOT `elif deployment_mode == "self_hosted"` — so any unexpected mode value still falls back to today's behavior. (2) Keep the **two URL-resolution paths separate**: the connection path resolves the URL from `conn.endpoint` (the new shared logic); the self-hosted `_build_one`/`_resolve_url` keeps reading `tenant_config().mcp_ado_organization` etc. (the deprecated-but-still-wired flat fields). Do NOT merge them — the self-hosted azdo/azure URL behavior must be byte-identical. Compute `roles = current_roles() if settings.auth_enabled else {"Admin"}` ONCE before the mode branch (shared implies auth on, so `current_roles()` there is correct).
