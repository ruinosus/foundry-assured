...
| Change bundle ingest, manifest handling, or wiki pipeline | Knowledge pipeline and docbundle contract | `app/knowledge/ingest_docbundles.py`, `app/knowledge/adapt_openwiki.py`, `app/knowledge/docbundle_schema.py`, `app/knowledge/wiki_builder.py` | `uv run python -m eval.docbundle_contract_test && uv run python -m eval.wiki_fidelity_test --component foundry-assured` |
