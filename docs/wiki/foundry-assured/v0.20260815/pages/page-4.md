...
- **Freshness**: `wiki_freshness_test.py` now grades one repository bundle, `foundry-assured`, against `.` while explicitly listing the old per-area bundles as retired history rather than failing them forever. The same module also exposes `--path-for` so the regeneration workflow can ask the gate which source tree a component covers instead of duplicating that map in YAML. [wiki_freshness_test.py](https://github.com/ruinosus/foundry-assured/blob/49e00ab92f3fe509be80db6dd5ceaca6088d2d37/apps/backend/eval/wiki_freshness_test.py#L23-L49) [wiki_freshness_test.py](https://github.com/ruinosus/foundry-assured/blob/49e00ab92f3fe509be80db6dd5ceaca6088d2d37/apps/backend/eval/wiki_freshness_test.py#L61-L76) [wiki_freshness_test.py](https://github.com/ruinosus/foundry-assured/blob/49e00ab92f3fe509be80db6dd5ceaca6088d2d37/apps/backend/eval/wiki_freshness_test.py#L84-L119)
...
- Wiki/docbundle pipeline changes: `uv run python -m eval.wiki_fidelity_test --component foundry-assured && uv run python -m eval.wiki_freshness_test`
