...
| [ADR-014](./ADR-014-runtime-prompt-scope-no-rebuild.md) | Runtime prompt scope **decouples from the image** — compose bind-mounts `apps/backend/.dna` over the baked copy; prompt edits = **restart, not rebuild**; Postgres source documented as the multi-replica evolution |
