...
| [ADR-013](./ADR-013-declarative-agent-prompts-dna.md) | Agent instructions = **declarative DNA scope** (`apps/backend/.dna/helpdesk/`, commit-pinned `dna-sdk`); `prompts.py` becomes a composition shim guarded by a byte-equivalence gate + prompt-invariant eval suite in CI |
