...
| ✅ **On-Behalf-Of (OBO)** for downstream calls as the signed-in user | `app/shared/auth.py`, `app/modules/knowledge/internal/secure_search.py` | [OBO flow](https://learn.microsoft.com/en-us/entra/identity-platform/v2-oauth2-on-behalf-of-flow) |
...
| ✅ **Multi-tenant bearer** (`MultiTenantAzureAuthorizationCodeBearer`) in shared mode; single-tenant otherwise | `app/shared/auth.py` | [fastapi-azure-auth](https://intility.github.io/fastapi-azure-auth/) |

## 2. Models, agents & retrieval

| Pattern (Microsoft indicates) | Where we do it | Proof |
|---|---|---|
| ✅ **Service identity calls models via a hosted agent**, not raw inference — the MI is authorized for `/agents/…`, but the Foundry data plane 403s a service principal on raw `/openai/v1/responses` (verified in-session; matches the [known Q&A](https://learn.microsoft.com/en-us/answers/questions/5779747/azure-ai-foundry-agent-returns-403-rbac-access-den)) 🔎 | `apps/hosted-{agent,techdocs,selfwiki,platform}` + the Live/Hosted toggle | [Hosted agents (preview)](https://learn.microsoft.com/en-us/azure/foundry/agents/concepts/hosted-agents) |
| ✅ **`FoundryChatClient` + `DefaultAzureCredential`** for KB-grounded agents | `app/modules/{grounded,platform_ops}/internal/*.py`, `apps/hosted-*/main.py` | [Agent types (agent-framework)](https://learn.microsoft.com/en-us/agent-framework/user-guide/agents/agent-types/azure-ai-foundry-agent) |
...
| ✅ **Foundry IQ knowledge base + agentic retrieval** (`AzureAISearchContextProvider`, `mode="agentic"`) | `app/modules/grounded/internal/d/techdocs.py`, `selfwiki.py`, ingest scripts | [Connect agents to Foundry IQ KBs](https://learn.microsoft.com/en-us/azure/ai-foundry/agents/how-to/tools/knowledge-retrieval?view=foundry) |
...
| ✅ **Bicep = control plane; data plane via scripts** (KB/memory not in Bicep) | `infra/*.bicep` vs `app/modules/knowledge/internal/*`, `cli/*` | [Control plane vs data plane](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/control-plane-and-data-plane) |
...
| ✅ **App Roles** (Admin/Author/Approver/Reader) in the token `roles` claim for app authorization | `app/shared/auth.py` `require_role`, `scripts/setup-app-roles.sh` | [App roles](https://learn.microsoft.com/en-us/entra/identity-platform/howto-add-app-roles-in-apps) |
| ✅ **Microsoft Graph app-only** for user + app-role management (no parallel user store) | `app/modules/admin/internal/graph.py`, `app/modules/admin/api_admin.py` | [Graph application permissions](https://learn.microsoft.com/en-us/graph/permissions-overview) |
