...
| ⚠️ **Structured citations/annotations** in the response — Microsoft indicates `url_citation`/`file_citation` annotations + Foundry IQ `id/source_file/content`. **We deviate**: `AzureAISearchContextProvider` injects docs as *context* (prose citations), so the response `annotations` come back empty (verified in-session). To get real annotations the retrieval must be wired as a tool/data-source. | `app/components/console/EvidencePanel.tsx` derives sources from the answer text (v1) | [Citations & annotations (reference app)](https://deepwiki.com/microsoft-foundry/foundry-agent-webapp/6.4-citations-and-annotations) |
