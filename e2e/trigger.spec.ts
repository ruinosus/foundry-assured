...
// full 2-user round-trip). Logs in as one user, asks TechDocs (Live), and writes any copilotkit
// run-stream RUN_ERROR line to artifacts/trigger-run-error.txt.

const PASS = process.env.TECHDOCS_TEST_PASSWORD ?? "";
const UPN = process.env.TECHDOCS_TEST_USER_A ?? "";
...
test("trigger techdocs grounded once (capture RUN_ERROR detail)", async ({ browser }) => {
  test.skip(!PASS || !UPN, "set TECHDOCS_TEST_USER_A + TECHDOCS_TEST_PASSWORD");
  ...
  try {
    ...
    await composer.fill("Como funciona a telemetria do TechDocs?");
