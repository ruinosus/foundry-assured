...
test("authenticated smoke — sign in + 4 domains + grounded helpdesk answer", async ({ page }) => {
  ...
  for (const domain of ["helpdesk", "techdocs", "selfwiki", "platform"]) {
  ...
  // 4) GROUNDED STRUCTURED CITATIONS (the 2026-07-01 slice): TechDocs in LIVE mode now runs the
  ...
  await ck.click();
  await ck.fill("Quais são os servidores MCP do TechDocs?");
  ...
  await shot(page, "techdocs-live-citations");
