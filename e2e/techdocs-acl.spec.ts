...
// Signs in as two Entra users against the app, asks TechDocs the same grounded question, and asserts
// the confidential doc is CITED for the CLEARED user (A) but NOT for the public-only user (B). This
// exercises the full UNIFIED grounded stack: MSAL login → the /d/techdocs route → backend /techdocs →
...
// live /techdocs); this spec confirms the same ACL surfaces correctly in the real browser UI.
//
// NOTE (unification): techdocs no longer has a Foundry hosted twin — grounded runs live-OBO only, so
// there is no "Live" toggle to click anymore (the earlier toggle step is now a harmless no-op). The
// route (/d/techdocs) and the winning assertion (cited SOURCE FILENAMES in .citation-src, never prose)
// are UNCHANGED.
//
// REQUIRES: the app reachable at E2E_BASE_URL running the unified grounded /techdocs path, and the two
// test users' creds. Skips cleanly when creds are absent.

const PASS = process.env.TECHDOCS_TEST_PASSWORD ?? "";
const USER_A = process.env.TECHDOCS_TEST_USER_A ?? "";
const USER_B = process.env.TECHDOCS_TEST_USER_B ?? "";
const CONFIDENTIAL = process.env.TECHDOCS_CONFIDENTIAL_SOURCE ?? "telemetry";
...
const PROBE =
  "Como funciona a telemetria e a observabilidade do TechDocs? Qual servidor MCP expõe a telemetria e como consultá-la?";
...
// What askTechDocsAs returns: the cited source filenames (for the ACL check) and — when the caller
...
// Sign in a specific user in a FRESH context (no shared MSAL cache), then ask TechDocs (Live) the
...
async function askTechDocsAs(
  ...
  try {
    ...
    await page.goto("/d/techdocs");
    await page.waitForLoadState("networkidle").catch(() => {});
    // Post-unification techdocs runs live-OBO only (no hosted twin), so there's usually no "Live"
    ...
    if (probeContent) {
      ...
      await shot(page, `techdocs-${tag}-citation-open`);
    }
...
test("techdocs ACL round-trip — A sees the confidential doc, B does not", async ({ browser }) => {
  test.skip(!PASS || !USER_A || !USER_B, "set TECHDOCS_TEST_USER_A/B + TECHDOCS_TEST_PASSWORD to run");
  test.setTimeout(10 * 60 * 1000); // two full logins (MFA) + two cold grounded answers

  const resultA = await askTechDocsAs(browser, USER_A, "A-cleared", /*probeContent*/ true);
  const resultB = await askTechDocsAs(browser, USER_B, "B-public");
